...
class SolveCamera(
	...
	fun solve(database: SnapshotsDatabase): Solution? {
		...
		for (cameraDelayInMs in -500..500 step 10) {
...
// 				LogManager.debug("Solved camera $angleDeg deg: rms=$rms camera=$camera")
		...
		LogManager.debug("Solved camera: $bestCamera delay=$bestCameraDelay wristInTracker=$bestWristInTracker")
	...
	private fun solveCamera(
		...
		val costFn = MultivariateVectorFunction { params ->
			...
			val wristInTracker = Vector3D(params[5], params[6], params[7])
			...
			// Prevent y from going below ground
			val y = camera.extrinsic.cameraOriginInWorld.y
// 			residuals[n - 1] = if (y < 0.0) y * 10000.0 else 0.0
		...
		val initial = doubleArrayOf(
			...
			1.0, // focal length scale
			0.0,
			0.0,
			0.0,
		)

// 		return Pair(buildCamera(initial, initialCamera), 100.0)

// 		val startCamera = buildCamera(initial, initialCamera)
// 		LogManager.debug("Start camera: $startCamera")
// 		LogManager.debug("Start camera Z: ${startCamera.extrinsic.cameraToWorld.sandwichUnitZ()}")
		...
		val result = optimizer.optimize(problem)

		val bestCamera = buildCamera(result.point.toArray(), initialCamera)
		val wristInTracker = Vector3D(result.point.getEntry(5), result.point.getEntry(6), result.point.getEntry(7))
...
// 		val cameraOriginInWorld = Vector3D(-0.11, 0.12, -2.34)
// 		val cameraOriginInWorld = Vector3D(2.44, 0.12, -1.00)
		val focalLengthScale = 1.0

		return Camera(
			CameraExtrinsic.fromCameraPose(cameraToWorld, cameraOriginInWorld),
			CameraIntrinsic(initialCamera.intrinsic.fx * focalLengthScale, initialCamera.intrinsic.fy * focalLengthScale, initialCamera.intrinsic.tx, initialCamera.intrinsic.ty),
			initialCamera.imageSize,
		)
	}
	...
	companion object {
		/**
		 * Approximate position of the wrist when holding the controller.
		 */
		// From Unity
// 		private val QUEST_3_LEFT_CONTROLLER_TIP = Vector3D(-0.008, -0.080, 0.128)
// 		private val QUEST_3_RIGHT_CONTROLLER_TIP = Vector3D(0.008, -0.080, 0.128)

		// From OVR??? +z is forward along handle, +y is up, +x is left
		private val QUEST_3_RIGHT_CONTROLLER_TIP = Vector3D.NULL // Vector3D(-0.008, 0.0, -0.128)

		private val FOCAL_LENGTH_SCALE_FACTOR = 0.8
	}
}
