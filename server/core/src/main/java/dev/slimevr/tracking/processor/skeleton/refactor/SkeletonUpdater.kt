...
class SkeletonUpdater(
	...
	class TrackerData(
		...
		companion object {
			fun fromSnapshot(snapshot: PositionalTracker) = TrackerData(
				snapshot.trackerToOVRWorld.toFloat(),
				snapshot.trackerOriginInOVRWorld.toFloat(),
			)

			fun fromSnapshot(snapshot: IMUTracker) = TrackerData(
				snapshot.trackerToArbitraryWorld.toFloat(),
				null,
			)
		}
	}
	...
	class TrackersData(
		...
		companion object {
			fun fromSnapshot(snapshot: TrackersSnapshot) = TrackersData(
				head = snapshot.controllers[TrackerPosition.HEAD]?.let(TrackerData::fromSnapshot),
				neck = snapshot.imuTrackers[TrackerPosition.NECK]?.let(TrackerData::fromSnapshot),
				upperChest = snapshot.imuTrackers[TrackerPosition.UPPER_CHEST]?.let(TrackerData::fromSnapshot),
				chest = snapshot.imuTrackers[TrackerPosition.CHEST]?.let(TrackerData::fromSnapshot),
				waist = snapshot.imuTrackers[TrackerPosition.WAIST]?.let(TrackerData::fromSnapshot),
				hip = snapshot.imuTrackers[TrackerPosition.HIP]?.let(TrackerData::fromSnapshot),
				leftUpperLeg = snapshot.imuTrackers[TrackerPosition.LEFT_UPPER_LEG]?.let(TrackerData::fromSnapshot),
				leftLowerLeg = snapshot.imuTrackers[TrackerPosition.LEFT_LOWER_LEG]?.let(TrackerData::fromSnapshot),
				leftFoot = snapshot.imuTrackers[TrackerPosition.LEFT_FOOT]?.let(TrackerData::fromSnapshot),
				rightUpperLeg = snapshot.imuTrackers[TrackerPosition.RIGHT_UPPER_LEG]?.let(TrackerData::fromSnapshot),
				rightLowerLeg = snapshot.imuTrackers[TrackerPosition.RIGHT_LOWER_LEG]?.let(TrackerData::fromSnapshot),
				rightFoot = snapshot.imuTrackers[TrackerPosition.RIGHT_FOOT]?.let(TrackerData::fromSnapshot),
				leftShoulder = snapshot.imuTrackers[TrackerPosition.LEFT_SHOULDER]?.let(TrackerData::fromSnapshot),
				leftUpperArm = snapshot.imuTrackers[TrackerPosition.LEFT_UPPER_ARM]?.let(TrackerData::fromSnapshot),
				leftLowerArm = snapshot.imuTrackers[TrackerPosition.LEFT_LOWER_ARM]?.let(TrackerData::fromSnapshot),
				leftHand = snapshot.controllers[TrackerPosition.LEFT_HAND]?.let(TrackerData::fromSnapshot),
				rightShoulder = snapshot.imuTrackers[TrackerPosition.RIGHT_SHOULDER]?.let(TrackerData::fromSnapshot),
				rightUpperArm = snapshot.imuTrackers[TrackerPosition.RIGHT_UPPER_ARM]?.let(TrackerData::fromSnapshot),
				rightLowerArm = snapshot.imuTrackers[TrackerPosition.RIGHT_LOWER_ARM]?.let(TrackerData::fromSnapshot),
				rightHand = snapshot.controllers[TrackerPosition.RIGHT_HAND]?.let(TrackerData::fromSnapshot),
			)
		}
	}
	...
	private fun updateSpineTransforms() {
		...
		if (hasSpineTracker) {
			val upperChestTracker = trackers.upperChest
			val chestTracker = trackers.chest
			val waistTracker = trackers.waist
			val hipTracker = trackers.hip
			if (upperChestTracker == null && chestTracker != null && waistTracker == null && hipTracker != null) {
				val chestRotation = chestTracker.rotation
				val hipRotation = hipTracker.rotation

				skeleton.upperChestBone.setRotation(chestRotation)
				skeleton.chestBone.setRotation(chestRotation)
				skeleton.waistBone.setRotation(chestRotation.interpR(hipRotation, 0.5f))
				skeleton.hipBone.setRotation(hipRotation)

				skeleton.chestTrackerBone.setRotation(chestRotation)
				skeleton.hipTrackerBone.setRotation(hipRotation)
			} else {
