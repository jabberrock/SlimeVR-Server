	...
	class TrackersData(
		...
		companion object {
			fun fromSnapshot(snapshot: TrackersSnapshot) = TrackersData(
				head = snapshot.trackers[TrackerPosition.HEAD]?.let(TrackerData::fromSnapshot),
				neck = snapshot.trackers[TrackerPosition.NECK]?.let(TrackerData::fromSnapshot),
				upperChest = snapshot.trackers[TrackerPosition.UPPER_CHEST]?.let(TrackerData::fromSnapshot),
				chest = snapshot.trackers[TrackerPosition.CHEST]?.let(TrackerData::fromSnapshot),
				waist = snapshot.trackers[TrackerPosition.WAIST]?.let(TrackerData::fromSnapshot),
				hip = snapshot.trackers[TrackerPosition.HIP]?.let(TrackerData::fromSnapshot),
				leftUpperLeg = snapshot.trackers[TrackerPosition.LEFT_UPPER_LEG]?.let(TrackerData::fromSnapshot),
				leftLowerLeg = snapshot.trackers[TrackerPosition.LEFT_LOWER_LEG]?.let(TrackerData::fromSnapshot),
				leftFoot = snapshot.trackers[TrackerPosition.LEFT_FOOT]?.let(TrackerData::fromSnapshot),
				rightUpperLeg = snapshot.trackers[TrackerPosition.RIGHT_UPPER_LEG]?.let(TrackerData::fromSnapshot),
				rightLowerLeg = snapshot.trackers[TrackerPosition.RIGHT_LOWER_LEG]?.let(TrackerData::fromSnapshot),
				rightFoot = snapshot.trackers[TrackerPosition.RIGHT_FOOT]?.let(TrackerData::fromSnapshot),
				leftShoulder = snapshot.trackers[TrackerPosition.LEFT_SHOULDER]?.let(TrackerData::fromSnapshot),
				leftUpperArm = snapshot.trackers[TrackerPosition.LEFT_UPPER_ARM]?.let(TrackerData::fromSnapshot),
				leftLowerArm = snapshot.trackers[TrackerPosition.LEFT_LOWER_ARM]?.let(TrackerData::fromSnapshot),
				leftHand = snapshot.trackers[TrackerPosition.LEFT_HAND]?.let(TrackerData::fromSnapshot),
				rightShoulder = snapshot.trackers[TrackerPosition.RIGHT_SHOULDER]?.let(TrackerData::fromSnapshot),
				rightUpperArm = snapshot.trackers[TrackerPosition.RIGHT_UPPER_ARM]?.let(TrackerData::fromSnapshot),
				rightLowerArm = snapshot.trackers[TrackerPosition.RIGHT_LOWER_ARM]?.let(TrackerData::fromSnapshot),
				rightHand = snapshot.trackers[TrackerPosition.RIGHT_HAND]?.let(TrackerData::fromSnapshot),
			)
		}
	}
