...
class CenterErrorVisitor(
	...
	override fun visitUpperLegTracker(
		...
		error(tracker, extraYaw(side, relaxedBodyAngles.upperLeg))
	...
	override fun visitLowerLegTracker(
		...
		error(tracker, extraYaw(side, relaxedBodyAngles.lowerLeg))
	...
	override fun visitFootTracker(
		...
		error(tracker, extraYaw(side, relaxedBodyAngles.foot))

	private fun error(tracker: Tracker, extraYaw: Angle = Angle.ZERO) =
		Angle.signedBetween(trackerYaw(tracker), centerYaw + extraYaw)
