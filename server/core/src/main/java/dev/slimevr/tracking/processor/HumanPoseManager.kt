...
class HumanPoseManager(val server: VRServer?) {
	...
	fun resetTrackersFull(
		...
		skeleton.resetTrackersFull(resetSourceName, trackerPositions, referenceTrackerPosition)
		// TODO: How does this change if reference tracker is not head?
		if (server != null) {
	...
	fun resetTrackersYaw(
		...
		skeleton.resetTrackersYaw(resetSourceName, trackerPositions, referenceTrackerPosition)
		if (server != null) {
			if (skeleton.headTracker == null && skeleton.neckTracker == null) {
				server.vrcOSCHandler.yawAlign(IDENTITY)
			} else {
	...
	fun resetTrackersMounting(
		...
		skeleton.resetTrackersMounting(resetSourceName, trackerPositions, referenceTrackerPosition)
	}
	...
	fun setStayAlignedConfig(config: StayAlignedConfig) {
		skeleton.setStayAlignedConfig(config)
	}
