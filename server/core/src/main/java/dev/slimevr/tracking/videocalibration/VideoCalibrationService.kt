...
import dev.slimevr.tracking.videocalibration.sources.WebRTCVideoSource
...
class VideoCalibrationService(
	...
	private val webcamSource: WebRTCVideoSource
	...
	init {
		webcamSource =
			WebRTCVideoSource(webcamService, debugOutput)
	...
	fun onTick() {
		...
		if (webcamSource.status.get() == WebRTCVideoSource.Status.DONE ||
