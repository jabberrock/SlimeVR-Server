			...
			is UDPPacket21UserAction -> {
				...
				when (packet.type) {
					UDPPacket21UserAction.RESET_FULL -> {
						...
						VRServer.instance.resetTrackersFull(RESET_SOURCE_NAME, setOf(), TrackerPosition.HEAD)
					}

					UDPPacket21UserAction.RESET_YAW -> {
						name = "Yaw reset"
						VRServer.instance.resetHandler.sendStarted(ResetType.Yaw)
						VRServer.instance.resetTrackersYaw(RESET_SOURCE_NAME, setOf(), TrackerPosition.HEAD)
					}
					...
					UDPPacket21UserAction.RESET_MOUNTING -> {
						...
						VRServer.instance.resetTrackersMounting(RESET_SOURCE_NAME, setOf(), TrackerPosition.HEAD)
					}
