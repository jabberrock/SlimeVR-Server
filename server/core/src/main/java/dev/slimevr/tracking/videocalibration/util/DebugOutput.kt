...
class DebugOutput(dir: Path) {
	...
	init {
		...
		webcamDir = dir.resolve("webcam")
		webcamDir.createDirectory()

		humanPosesDir = dir.resolve("poses")
		humanPosesDir.createDirectory()

		skeletonOffsetsDir = dir.resolve("skeleton_offsets")
		skeletonOffsetsDir.createDirectory()

		reconstructionDir = dir.resolve("reconstruction")
	...
	fun saveHumanPoseImage(timestamp: Duration, image: BufferedImage, humanPoseSnapshot: HumanPoseSnapshot) {
		...
		ImageIO.write(poseImage, "jpg", humanPoseImage(timestamp).toFile())
	}
	...
	fun saveReconstruction(
		...
		for ((i, frame) in frames.withIndex()) {
			val (trackersSnapshot, humanSnapshot) = frame

			val image = ImageIO.read(humanPoseImage(humanSnapshot.timestamp).toFile())

			val g = image.createGraphics()
			drawBones(g, humanSnapshot)

			val leftShoulder = humanSnapshot.joints[CocoWholeBodyKeypoint.LEFT_SHOULDER]
			val rightShoulder = humanSnapshot.joints[CocoWholeBodyKeypoint.RIGHT_SHOULDER]
			val leftHip = humanSnapshot.joints[CocoWholeBodyKeypoint.LEFT_HIP]
			val rightHip = humanSnapshot.joints[CocoWholeBodyKeypoint.RIGHT_HIP]
			if (leftShoulder != null && rightShoulder != null && leftHip != null && rightHip != null) {
				...
				for ((i, trackerPosition) in UPPER_BODY_TRACKERS.withIndex()) {
					...
					val r = trackerReset.toBoneRotation(tracker.rawTrackerToWorld)
					drawAxis(g, r, root, camera, AXIS_SCALE)
				}
			}

			val headTracker = trackersSnapshot.trackers[TrackerPosition.HEAD]
			if (headTracker != null) {
				val nose = humanSnapshot.joints[CocoWholeBodyKeypoint.NOSE] ?: continue

				val r = headTracker.rawTrackerToWorld
				drawAxis(g, r, nose, camera, AXIS_SCALE)
			}

			for ((trackerPosition, trackerSnapshot) in trackersSnapshot.trackers) {
				...
				val r = trackerReset.toBoneRotation(trackerSnapshot.rawTrackerToWorld)
		...
		if (ffmpeg != null && Path(ffmpeg).exists()) {
			...
			for (i in 1 until frames.size) {
				val f1 = frames[i - 1]
				val f2 = frames[i]
				val interval = f2.second.timestamp - f1.second.timestamp
	...
	fun saveSkeletonOffsets(
		...
		for ((i, frame) in frames.withIndex()) {
			...
			val webcamImage = ImageIO.read(webcamImage(humanSnapshot.timestamp).toFile())

			val image = BufferedImage(webcamImage.width * 2, webcamImage.height, BufferedImage.TYPE_INT_RGB)

			val g = image.createGraphics()
			g.drawImage(webcamImage, 0, 0, webcamImage.width, webcamImage.height, 0, 0, webcamImage.width, webcamImage.height, null)
			g.drawImage(webcamImage, webcamImage.width, 0, webcamImage.width * 2, webcamImage.height, 0, 0, webcamImage.width, webcamImage.height, null)

			val trackersData = SkeletonUpdater.TrackersData.fromSnapshot(trackersSnapshot)

			// Draw initial
			SkeletonUpdater(skeleton, trackersData, config, initialSkeletonOffsets).update()
			drawSkeleton(g, skeleton, camera, Vector2D.NULL)

			// Draw optimized
			SkeletonUpdater(skeleton, trackersData, config, optimizedSkeletonOffsets).update()
			drawSkeleton(g, skeleton, camera, Vector2D(webcamImage.width.toDouble(), 0.0))
	...
	private fun drawSkeleton(g: Graphics2D, skeleton: Skeleton, camera: Camera, offset: Vector2D) {
		...
		while (bones.isNotEmpty()) {
			...
			if (head != null && tail != null) {
				drawLine(g, head + offset, tail + offset, BONE_COLOR, BONE_STROKE)
			}
