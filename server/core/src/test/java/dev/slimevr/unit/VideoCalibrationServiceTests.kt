...
class VideoCalibrationServiceTests {
	...
	@Test
	fun e2eCalibration() {
		val dataFolder = Path("C:\\Users\\yilan\\AppData\\Roaming\\dev.slimevr.SlimeVR\\VideoCalibrationTest15")
		...
		val debugOutput = DebugOutput(Path("C:\\SlimeVR\\VideoCalibrationTest"))

		val humanPoseSource = HumanPoseSource(imageSnapshots, debugOutput)
...
// 			TrackerPosition.RIGHT_UPPER_ARM, // FIXME: Data may not contain these positions
		...
		dataFolder.resolve("webcam").toFile().copyRecursively(debugOutput.webcamDir.toFile(), true)
		...
		CoroutineScope(Dispatchers.IO).launch {
			...
			val webcamFolder = dataFolder.resolve("webcam")
